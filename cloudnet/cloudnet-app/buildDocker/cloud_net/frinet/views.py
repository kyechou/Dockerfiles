...
def get_posts(request):
    ...
    for u in all_users:
        # load from cache
        cached_posts = cache.get(u)
        if cached_posts:
            print ("[Cache Hit] " + str(u) + ": " + str(cached_posts))
            all_posts += cached_posts
        else:
            print ("[Cache Miss] " + str(u))
            retrieved_posts = [p for p in Post.objects.filter(user=u)]
            cache.set(u, retrieved_posts)
            print ("[Cache Load] " + str(u) + ": " + str(retrieved_posts))
            all_posts += retrieved_posts
...
def make_post(request):
    ...
    post.save()
    cache.delete(user);
    print ("[Cache Invalidate] " + str(user))
